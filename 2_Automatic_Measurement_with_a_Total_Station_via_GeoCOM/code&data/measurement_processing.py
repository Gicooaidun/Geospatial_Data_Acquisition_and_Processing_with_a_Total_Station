...
def arithmetic_check2(x_jk, x_jk_slash, num_set, num_point):
    ...
    for j in range(0, num_set):
        ...
        if check2[j] < 200:
            ...
            phase_correction = 400

        # Check if the result is not equal to 0 and not within a small tolerance (0.001)
        if check2[j] != 0 and abs(check2[j] - phase_correction) > 0.001:
            return False
...
def process_pandas(dataframe, num_set, num_point, filename_without_extension):
    """
    This function reads data from a pandas dataframe, processes it to calculate measurements and residuals and
    ...
    1. Read the dataframe and convert it into a NumPy ndarray.
    ...
    """
    horizontal_angles = dataframe['Hz_gon'].values
    ...
    if arithmetic_check1(x_jk1, x_jk2, x_jk, num_set) and arithmetic_check2(x_jk, x_jk_slash, num_set,
                                                                            num_point) and arithmetic_check3(x_jk_slash,
                                                                                                             x_k_mean,
                                                                                                             d_jk,
                                                                                                             num_set) and arithmetic_check4(
            r_jk, num_set) and arithmetic_check5(x_jk_slash, x_k_mean, num_set) and arithmetic_check6(d_jk):
    ...
    log_filename = 'Output_log/' + filename_without_extension + '_iso_17123_3' + '.log'
    ...
    log_file.write('Input File: ' + filename_without_extension + '\n')
    ...
    print('Residual Sum of Squares: ' + str(r_square_sum) + ' gon\u00b2')
    log_file.write('Residual Sum of Squares: ' + str(r_square_sum) + ' gon\u00b2' + '\n')

    print('Measurements Standard Deviation: ' + str(std) + ' gon')
    log_file.write('Measurements Standard Deviation: ' + str(std) + ' gon' + '\n')
